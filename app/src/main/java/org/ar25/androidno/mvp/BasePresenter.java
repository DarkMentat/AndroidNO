...
import org.ar25.androidno.util.ArrayUtils;
...
public abstract class BasePresenter<V extends MvpView> implements MvpPresenter<V> {
    ...
    public BasePresenter(PermissionManager permissionManager) {
    ...
    @Override public final void attach(V view) {
        ...
        presenterViews.onNext(this.view);
    ...
    @Override public final void destroy() {
        views.onCompleted();
        presenterViews.onCompleted();

        changLifecycleEvent(Lifecycle.DESTROY);
        onDestroy();
    }
     ...
     * state (@see attach, detach). If event is emitted while your view is detached, this event will
     * be reemited again when your view will be attached. If view will be destroyed after detach,
     * the subscriber will be unsubscribed. CAUTION: don't use this transformer for source
     * Observables that emmit more that one onNext of useful data. Group elements before passing
     * here {@see toList}.
     *
     * @return a Transformer that emits items emitted by source Observable. Always remembers the
     * last onNext event, if such emitted.
     */
        ...
        return source -> {
            ...
            return views.filter(v -> v != null).switchMap(v -> subject.filter(item -> {
                ...
                return ArrayUtils.lastIndexOf(items, item) == lastOnNextIndex;
                    ...
                    .compose(bindUntilEvent(Lifecycle.DESTROY));
        };
    }

    /**
     * @param event lifecycle event {@link Lifecycle.Event}
     * @return a Transformer that unsubscribes from the source Observable when lifecycle event
     * equals to {@see event}
     */
     ...
     * @return Transformer returns {@link PermissionManager.PermissionObtained} only if permission
     * is granted
     */
    protected <T> Observable.Transformer<T, T> requirePermissions(String permission) {
        return requirePermissions(new String[] {permission});
    }

    protected <T> Observable.Transformer<T, T> requirePermissions(String[] permissions) {
